...
build:
	...
	ts-node --compiler-options '{"module": "CommonJS"}' -p -e "JSON.stringify(require('./src/config'))" > static/provider.json
	sed -i '' 's/^{"default":{/{/g' static/provider.json
	sed -i '' "s/}}$$/}/g" static/provider.json

	GATSBY_BUILD_TIME=$(GATSBY_BUILD_TIME) gatsby build --prefix-paths
